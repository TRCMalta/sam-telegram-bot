 ...
 *
 * Hermes is ~23x cheaper per input token than Claude Sonnet, and these
 ...
 * Beverly a reply.
 *
 * 2026-09-04: nousresearch/hermes-4-70b returns HTTP 404 from OpenRouter's
 * Nebius upstream ("model does not exist") even though it's listed in
 * OpenRouter's catalogue — Nebius stopped serving that specific slug. Swapped
 * light-model default to hermes-3-llama-3.1-70b (DeepInfra), verified live.
 * hermes-4-405b still works fine on Nebius, left as-is for the heavy model.
 *
 * Config (env):
 *   OPENROUTER_API_KEY  — required to enable routing; absent = everything on Claude
 *   HERMES_MODEL        — default nousresearch/hermes-3-llama-3.1-70b
 *   HERMES_HEAVY_MODEL  — default nousresearch/hermes-4-405b (harder extraction)
 *   HERMES_TIMEOUT_MS   — default 20000
 */
const OPENROUTER_API_KEY = process.env.OPENROUTER_API_KEY;
const HERMES_MODEL       = process.env.HERMES_MODEL || "nousresearch/hermes-3-llama-3.1-70b";
