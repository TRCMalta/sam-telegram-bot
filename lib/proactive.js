 ...
 * @param config      { morningHour, weeklyDay, weeklyHour, chaseHour, staleDay, staleHour }
 */
export function startProactive({ send, sendVoice, ask, getCalendar, config = {} }) {
  const cfg = {
  ...
  async function tick() {
    const { dateIso, hour, minute, weekday } = maltaClock();

    // ── Morning briefing ──
    if (hour === cfg.morningHour && minute < 5) {
    ...
    // ── Friday debrief ──
    if (weekday === cfg.weeklyDay && hour === cfg.weeklyHour && minute < 5) {
    ...
    // ── Overdue chase ──
    if (hour === cfg.chaseHour && minute < 5) {
      await once(`chase:${dateIso}`, async () => {
        ...
        if (text) { await send(text); await markChased(chaseable.map((i) => i.id)); }
      });
    }

    // ── Stale relationships ──
    if (weekday === cfg.staleDay && hour === cfg.staleHour && minute < 5) {
      await once(`stale:${dateIso}`, async () => {
        ...
        if (text) { await send(text); await markRelationshipsAlerted(stale.map((s) => s.name)); }
      });
    }
